...
Примеры:
  ...
  text-cli stats --input data.txt --top 10
    ...
    stats_parser.add_argument(
        "--input", "-i", type=Path, required=True, help="Путь к файлу для анализа"
    )
    stats_parser.add_argument(
        "--top",
        "-t",
        type=int,
        default=5,
        help="Количество самых частотных слов (по умолчанию: 5)",
    )
    ...
    try:
        ...
        print(
            f"Ошибка: указан путь к папке, а не к файлу — '{input_path}'",
            file=sys.stderr,
        )
        sys.exit(1)
        ...
        print(
            f"Ошибка: файл '{input_path}' не может быть прочитан как текст в кодировке UTF-8.",
            file=sys.stderr,
        )
        sys.exit(1)
