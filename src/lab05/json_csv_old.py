...
def json_to_csv(json_path: str, csv_path: str) -> None:
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as c:
        ...
        writer = csv.DictWriter(
            c, fieldnames=sorted(keys)
        )  # создаём переменную порядок колонок алфавитный
        writer.writeheader()
        for entry in data:
...
def csv_to_json(csv_path: str, json_path: str) -> None:
    ...
    with open(json_path, "w", encoding="utf-8") as j:  # открытие файла в режиме записи
        json.dump(
            csv_data, j, ensure_ascii=False, indent=4
        )  # запись в файл ensure_ascii=False - запрет на показ не нужных символах
